    ...
    for action in actionList:
      ...
      of "preparePWA":
        ...
        if not pwa.isNil:
          ...
          ##  generate asset file for pwa
          let assetFile = open(srcFrontendDir.joinPath("assets.nim"), fmWrite)
          assetFile.write(
            "##\n" &
            "## this is auto generated from pwa nakefile.json\n" &
            "## should not do any changes here\n" &
            "## will rewrite depend on pwa assets configuration\n" &
            "##\n" &
            &"let assetsList* = {assets.pretty}\n"
          )
          assetFile.close()
